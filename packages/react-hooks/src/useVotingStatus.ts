...
function getStatus (api: ApiPromise, bestNumber: BlockNumber, votes: Votes, numMembers: number, section: 'council' | 'membership' | 'technicalCommittee' | 'alliance'): State {
  if (!votes.end) {
    return {
      hasFailed: false,
      hasPassed: false,
...
export function useVotingStatus (votes: Votes | null | undefined, numMembers: number, section: 'council' | 'membership' | 'technicalCommittee' | 'alliance'): State {
  const { api } = useApi();
